...
class HtmlEditorWidget extends StatefulWidget {
  ...
  final Function? onHtmlEditorInitialized;
...
class _HtmlEditorWidgetMobileState extends State<HtmlEditorWidget> {
              ...
              Expanded(
                child: InAppWebView(
                  ...
                  onLoadStop: (InAppWebViewController controller, Uri? uri) async {
                    ...
                    if (widget.onHtmlEditorInitialized != null) {
                      widget.onHtmlEditorInitialized!();
                    }
                  },
                ),
              ),
  ...
  void addJSCallbacks(Callbacks c) {
    ...
    if (c.onMouseDown != null) {
      widget.controller.editorController!.evaluateJavascript(source: """
          \$('#summernote-2').on('summernote.mousedown', function(e) {
            console.log("data check",e.target);
    ...
    if (c.onMouseUp != null) {
      widget.controller.editorController!.evaluateJavascript(source: """
          \$('#summernote-2').on('summernote.mouseup', function(e) {
            console.log("data check",e.target);
    ...
    \$('#summernote-2').on('summernote.mousedown', function(e) {
          document.addEventListener("click", function (event) {
            if (event.target.tagName === "A" || event.target.tagName === "VIDEO") {
              window.flutter_inappwebview.callHandler('onLinkTap', event.target.getAttribute("href").toString());;
            }
          });
      });
      """);
    }
    if (c.onVideoTap != null) {
      widget.controller.editorController!.evaluateJavascript(source: """
    \$('#summernote-2').on('summernote.mousedown', function(e) {
          document.addEventListener("click", function (event) {
            console.log(event.target.tagName);
            if (event.target.tagName === "VIDEO") {
              window.flutter_inappwebview.callHandler('onVideoTap', event.target.getAttribute("src").toString());;
            }
          });
      });
      """);
  ...
  void addJSHandlers(Callbacks c) {
    ...
    if (c.onLinkTap != null) {
      widget.controller.editorController!.addJavaScriptHandler(
          ...
          callback: (contents) {
            print('Here i came ${contents.first.toString()}');
            c.onLinkTap!.call(contents.first.toString());
          });
    }
    if (c.onVideoTap != null) {
      widget.controller.editorController!.addJavaScriptHandler(
          handlerName: 'onVideoTap',
          callback: (contents) {
            print('Here i came ${contents.first.toString()}');
            c.onVideoTap!.call(contents.first.toString());
          });
    }
    if (c.onChangeCodeview != null) {
      widget.controller.editorController!.addJavaScriptHandler(
