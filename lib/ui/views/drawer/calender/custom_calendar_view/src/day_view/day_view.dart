...
class DayViewState<T extends Object?> extends State<DayView<T>> {
  ...
  late List<CalendarEventData<T>> eventsList;

  List? eventsOverlapping;

  bool? hasOverlappingEvents;
  ...
  void initState() {
    ...
    _setDateRange();
    _currentDate = (widget.initialDay).withoutTime;
  ...
  Widget _defaultEventTileBuilder(DateTime date, List<CalendarEventData<T>> events, Rect boundary, DateTime startDuration, DateTime endDuration) {
    if (events.isNotEmpty) {
      eventsOverlapping = events.map((e) {
        hasOverlappingEvents = controller.getEventsOnDay(widget.initialDay).where((e1) {
          final bool isInBetween = e1.startTime.isAfter(e.startTime) && e1.startTime.isBefore(e.endTime);
          final bool isEndingBeforeThis = e1.endTime.isAfter(e.startTime) && e1.endTime.isBefore(e.endTime);
          final bool isEndingAfterThis = e1.startTime.isBefore(e.startTime) && e1.endTime.isAfter(e.endTime);
          hasOverlappingEvents = (isInBetween || isEndingBeforeThis || isEndingAfterThis);
          return hasOverlappingEvents!;
        }).isNotEmpty;
        print(hasOverlappingEvents);
      }).toList();
      print(eventsOverlapping?.length);
      ...
      return RoundedEventTile(
          ...
          hasOverlappingEvents: hasOverlappingEvents);
    } else {
