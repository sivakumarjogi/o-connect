...
class WeekViewState<T extends Object?> extends State<WeekView<T>> {
  ...
  void initState() {
    ...
    _currentWeek = (widget.initialDay).withoutTime;
                  ...
                  child: DecoratedBox(
                    ...
                    child: SizedBox(
                      ...
                      child: PageView.builder(
                        ...
                        itemBuilder: (_, index) {
                          final dates = DateTime(_minDate.year, _minDate.month, _minDate.day + (index * DateTime.daysPerWeek)).datesOfWeek(start: widget.startDay);

                          return RefreshIndicator(
                            key: refreshKey,
                            onRefresh: () {
                              return value.fetchEventsInDay(context);
                            },
  ...
  Widget _defaultWeekDayBuilder(DateTime date) {
    return Center(
      child: Column(
        ...
        children: [
          Text(
            widget.weekDayStringBuilder?.call(date.weekday - 1) ?? Constants.weekTitles[date.weekday - 1],
            style: const TextStyle(color: LightThemeColors.greyTextColor, fontWeight: FontWeight.w500),
          ),
          Text(
            widget.weekDayDateStringBuilder?.call(date.day) ?? date.day.toString(),
            style: TextStyle(color: (DateTime.now().day == date.day) ? AppColors.mainBlueColor : LightThemeColors.greyTextColor, fontWeight: FontWeight.w500),
          ),
        ],
      ),
    );
  ...
  Widget _defaultWeekNumberBuilder(DateTime date) {
    ...
    final thursday = daysToAdd > 0 ? date.add(Duration(days: daysToAdd)) : date.subtract(Duration(days: daysToAdd.abs()));
  ...
  Widget _defaultTimeLineBuilder(DateTime date) {
    final timeLineString = widget.timeLineStringBuilder?.call(date) ?? "${((date.hour - 1) % 12) + 1} ${date.hour ~/ 12 == 0 ? "AM" : "PM"}";
    return Transform.translate(
      ...
      child: Padding(
        ...
        child: Text(
          ...
          style: const TextStyle(fontSize: 15.0, color: LightThemeColors.greyTextColor),
        ),
      ),
    );
  }
  ...
  /// Default timeline builder. This builder will be used if
  /// [widget.eventTileBuilder] is null
  Widget _defaultEventTileBuilder(DateTime date, List<CalendarEventData<T>> events, Rect boundary, DateTime startDuration, DateTime endDuration) {
    if (events.isNotEmpty) {
      return RoundedEventTile(
        ...
        isWeek: true,
        startDateTime: events[0].startTime,
        endDateTime: events[0].endTime,
      );
    } else {
      return Container();
    }
