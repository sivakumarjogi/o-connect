...
class FlipWidget<T> extends StatefulWidget {
  const FlipWidget({
    ...
    this.onDone,
    this.startCount = 0,
  })  : assert(hingeWidth == 0.0 && hingeLength == 0.0 || hingeWidth != 0.0 && hingeLength != 0.0),
  ...
  @override
  State<FlipWidget<T>> createState() => flipType == FlipType.middleFlip ? _MiddleFlipWidgetState<T>() : _SpinFlipWidgetState<T>();
}

abstract class _FlipWidgetState<T> extends State<FlipWidget<T>> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) => AnimatedBuilder(animation: _flipAnimation, builder: _buildDisplay);
  ...
  Widget firstFlatPanel(AxisDirection direction) => direction == AxisDirection.up || direction == AxisDirection.left ? _firstPanelChild1 : _firstPanelChild2;

  Widget secondFlatPanel(AxisDirection direction) => direction == AxisDirection.up || direction == AxisDirection.left ? _secondPanelChild2 : _secondPanelChild1;
...
class _MiddleFlipWidgetState<T> extends _FlipWidgetState<T> {
  ...
  Widget buildFirstFlipPanel() {
    ...
    return movingPanel == null ? flatPanel : Stack(children: [flatPanel, movingPanel]);
  }
  ...
  Transform? transform2FirstPanel(AxisDirection direction) {
    ...
    final isUpOrLeft = direction == AxisDirection.up || direction == AxisDirection.left;
    if (isUpOrLeft != isPastMiddle) return null;

    final isVertical = widget.axis == Axis.vertical;
    final sign = isVertical ? 1.0 : -1.0;
    final rotation = sign * (isUpOrLeft ? math.pi - _flipAnimation.value : _flipAnimation.value);

    final transform = Matrix4.identity()..setEntry(3, 2, _perspectiveAnimation.value);
  ...
  Transform? transform2SecondPanel(AxisDirection direction) {
    ...
    final rotation = sign * (isUpOrLeft ? _flipAnimation.value : math.pi - _flipAnimation.value);
...
class _SpinFlipWidgetState<T> extends _FlipWidgetState<T> {
  ...
  Transform? transform2FirstPanel(AxisDirection direction) {
    ...
    final sign = isVertical ? 1.0 : -1.0;
    final rotation = sign * (isUpOrLeft ? -_flipAnimation.value : _flipAnimation.value) + (isPastMiddle ? math.pi : 0.0);
...
class _WidgetClipper {
  ...
  Widget makeFirstClip(Widget widget, Axis axis) => axis == Axis.horizontal ? makeLeftClip(widget) : makeUpperClip(widget);

  Widget makeSecondClip(Widget widget, Axis axis) => axis == Axis.horizontal ? makeRightClip(widget) : makeLowerClip(widget);
